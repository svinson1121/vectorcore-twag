...
const (
	...
	minGTPv2Sequence uint32 = 0x000001
	maxGTPv2Sequence uint32 = 0x7fffff
)
...
func (m gtpv2Message) encode() ([]byte, error) {
	if m.Sequence < minGTPv2Sequence || m.Sequence > maxGTPv2Sequence {
		return nil, fmt.Errorf("gtpv2 sequence %d outside StarOS-compatible range 0x000001-0x7fffff", m.Sequence)
	}
	headerLen := 8
...
type gtpv2SequenceAllocator struct {
	seq atomic.Uint32
}

func newGTPv2SequenceAllocator() *gtpv2SequenceAllocator {
	a := &gtpv2SequenceAllocator{}
	a.seq.Store(randomInitialGTPv2Sequence())
	return a
}

func randomInitialGTPv2Sequence() uint32 {
	var b [4]byte
	if _, err := rand.Read(b[:]); err != nil {
		return minGTPv2Sequence
	}
	n := binary.BigEndian.Uint32(b[:]) & maxGTPv2Sequence
	if n == 0 {
		return minGTPv2Sequence
	}
	return n
...
func (a *gtpv2SequenceAllocator) next() uint32 {
	for {
		cur := a.seq.Load()
		next := cur + 1
		if next == 0 || next > maxGTPv2Sequence {
			next = minGTPv2Sequence
		}
